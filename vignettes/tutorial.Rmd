...
One advantages of using GTE is that it helps investigators obtain the desired
power and significance level with smaller sample size compared to univariate
analysis. 

Here, we estimate the sample sizes needed in each arm of the group based on univariate
analysis to obtain 90% power and 0.05 significance level using the same 
hypothetical summary data.
...
We would need 1884 participants in each arm in outcome 1, 210 participants in 
each arm in outcome 2, and 53 participants in each arm in outcome 3. 

As the result shows, sample sizes based on univariate analysis
are much greater compared to sample size estimation based on GTE.
...
The example dataset is the same hypothetical historical data from
previous section with 3 endpoints, first 40 rows contain observations in
group 1 and rest 60 rows contain observations in group 2.

```{r}
head(data)
```

We can use GST.parameter function to calculate some useful parameters
like treatment effect on individual outcomes, standard deviation of
global treatment effect and etc.
...
The p-value of the GST is obtained by comparing the value of Z to the
critical values from the normal distribution. For example, Z = 1.645
corresponds to p = 0.05 for the one-sided test and p = 0.10 for the
two-sided test; Z = 1.960 corresponds to p = 0.025 for the one-sided
test and p = 0.05 for the two-sided test.

Alternatively, GST.pvalue function could directly provide one/two-sided
