...
## Download GST package
```{r}
# install.packages("devtools")
# library(devtools)
# devtools::install_github("jh277/GST")
```
...
We can calculate the treatment effect $\theta$ of individual outcomes based on $\theta = 2\Phi\left(\frac{\mu_1 - \mu_2}{\sqrt{\sigma_1^2 + \sigma_2^2}}\right) - 1$ [@GTE]
...
In previous section, we obtained the treatment effect $\theta$ for each outcome, we can then calculate the global treatment effect by taking the average $\bar{\theta}$ of these $\theta$ values.

We can also derive an estimation of the standard deviation $\sigma^2 = \min\left\{\frac{1}{12}, \frac{1 - \min_{1 \leq v \leq K} (\theta_va^2)}{4}\right\}$. [@samplesize]

Plug these values into the function, we can get sample size estimation for each group.
